...
$(function () {
  ...
  // PCメニュー
  // メニューの位置を取得
  posMenu = $('#HEADER_MENU').offset().top;
  // スクロール時の処理
  $(window).scroll(function(){
    // ウィンドウのサイズを取得
    var winWSize = $(window).width();
    // メニューの初期位置より下にスクロールし、かつウィンドウサイズが767pxより大きい場合
    if ($(window).scrollTop() > scrPos && winWSize > winSpSize) {
      // メニューを追従させる
      $('.header-nav').addClass('p-fix');
    } else {
      // メニューの初期位置より上にスクロールした場合
      // メニューを固定する
      $('.header-nav').removeClass('p-fix');
    }
  });
